...
func main() {
	...
	if *flOneTime == false && *flWait != 0 {
		if *flInterval == 0 {
			log.V(0).Infof("The --wait flag is deprecated, please use --interval instead.")
			*flInterval = *flWait
		} else {
	...
	for {
		...
		if changed, err := syncRepo(ctx, *flRepo, *flBranch, *flRev, *flDepth, *flRoot, *flDest); err != nil {
			...
			log.V(0).Info("waiting before retrying", "waitTime", intervalTime(*flInterval))
			cancel()
		...
		failCount = 0
		log.V(1).Info("next sync", "wait_time", intervalTime(*flInterval))
		cancel()
		time.Sleep(intervalTime(*flInterval))
	}
}
